...
\usepackage{graphicx}
% Use Computer Modern monospace instead of Courier (avoids needing the pcr
% font package, which BasicTeX doesn't bundle).
...
%---- Convenience macros ------------------------------------------------------
\newcommand{\bev}{\mathbf{F}_{\mathrm{BEV}}}
\newcommand{\bevt}{\mathbf{F}_{\mathrm{BEV},t}}
\newcommand{\unet}{f_\psi}
\newcommand{\corrturns}{\mathrm{corr}_{\mathrm{turns}}}
...
\begin{equation}
  N_t(i, j) = \mathbb{I}\left[\|\bevt(i, j, :)\|_0 > 0\right] \cdot (1 - Y_t(i, j)).
\end{equation}
...
\begin{equation}
  \mathcal{L} = -\frac{1}{|Y_t|} \sum_{(i,j) \in Y_t} \log \sigm(\unet(\bevt)_{ij})
              -\frac{1}{|N_t|} \sum_{(i,j) \in N_t} \log (1 - \sigm(\unet(\bevt)_{ij})).
\end{equation}
