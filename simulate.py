...
class MPPIPlanner:
    def __init__(self, K=1000, T=8, sigma=0.3, lam=0.1,
                 bev_w=64, bev_h=64, max_steer=1.0,
...
def score_candidates(model, bev, candidates, device, gt_steer=None, prototype=None, n_steps=8, feat_dim=384):
    """
    ...
    by = np.clip(candidates[:, :, 0].astype(int), 0, 63)
    bx = np.clip(candidates[:, :, 1].astype(int), 0, 63)
    feats = bev[by, bx, :].reshape(K, n_steps * feat_dim)

    with torch.no_grad():
        t = torch.from_numpy(feats.astype(np.float32)).to(device)
        embeds = model.encode(t)          # (K, 128) — L2-normalised

        if prototype is not None:
            # Real-car mode: score by cosine similarity to prototype good-driving embedding
            scores = (embeds * prototype).sum(dim=-1).cpu().numpy()
        elif gt_steer is not None:
...
def run(args):
    ...
    ax_plan = fig.add_subplot(1, 3, 2)
    ax_hist = fig.add_subplot(1, 3, 3)

    for ax in [ax_bev, ax_plan, ax_hist]:
    ...
    for i, frame in enumerate(frames):
        ...
        ax_hist.plot(steer_hist[-w:], color='#76c7ff', linewidth=1.2, label='steering')
